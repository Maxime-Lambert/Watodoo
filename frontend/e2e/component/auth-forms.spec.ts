...
test.describe('Auth forms', () => {
  ...
  test('login shows a generic error message on invalid credentials', async ({ page }) => {
    await page.route('**/auth/login', async (route) => {
      await route.fulfill({
        status: 401,
        contentType: 'application/json',
        body: JSON.stringify({ title: 'Unauthorized', detail: 'Email ou mot de passe incorrect.' }),
      })
    })
  ...
  test('submit button is disabled while the request is pending', async ({ page }) => {
    await page.route('**/auth/login', async (route) => {
